...
When I figure out how to make this available in a repository.
Find and install it from (list-packages) or find a more permanent solution and:
...
If disabled it will restore the remembered settings.
...
The overall Vim modal interface is roughly preserved.
...
Once you have made that choice you might get used to prioritize it for pasting lines too.
...
This is also consistent with other lower case keybindings operating on the current line number like =cc= or =yy=.
...
I bind (transpose-chars) to =gs=.
That does roughly the same thing including the motion but it doesn't involve the kill ring.
