...
Supposedly this is convertible to GPL 3 but I'm not sure.
Pull request?
...
Download evil-cursor-between-mode.el, open and install it with (package-install-from-buffer).
...
With this mode (evil-append) is just move forward (=l=) and enter insert-state (=i=).
That is how you think if you adopt the mode's cursor model.

Therefore I swap evil-mode's =a= and =A=.

It is useful to access insert-state at the end of the line with the new =a=.
...
When all the commands operating on other lines are layered, you might start to adopt a new philosophy.
...
Remaining upper case bindings can be replaced by using a motion (e.g. =I= is =0i=).

With this mode you can embrace the line nugget idea and replace layers with motions.
If you want to open the next line you can use =jo= or =a<RET>= which are both two key presses, but so are the new =<shift>o=.
...
Over time you will develop muscle memory, which make you end up in the right place for using fewer key strokes.
...
The main drawback is that it will be harder to work efficiently with an unconfigured Vi, Vim or evil-mode editor.
...
This type of behavior where you have an implicit motion might feel unnatural in this mode.
...
Note that Vim's behavior is often available so e.g. =~= will work on the (char-after) and move forward.
...
In this mode there is no way to toggle back and forth between the second set of delimiters when the two sets are touching.
...
If you don't use your editor every day I think this is easier to pick up and more similar to other editing experiences.
...
I'm unsure how I submit this to a repository.
I think I have done everything related to formatting.

I have no formal background in programming and just picked up lisp, because I wrote a book in Emacs (org-mode/LaTeX/tikz).

My problem is that I have never used git as anything but an advanced backup.
The way I use it is to browse old stuff with (magit) and copy/paste what I need.
I think I can handle a pull request as maintainer, but anything more advanced is beyond my current skill set.
