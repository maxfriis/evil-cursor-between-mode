...
Somehow make evil-cursor-between-mode available to Emacs.
...
I like almost all my keybindings dealing with the current line to be lower case.
...
If you need to work with the next line you need to press more keys.
...
That is a real cost, since combining a command and motion can sometimes be utilized with a prefix.
...
(transpose-chars) does roughly the same thing including the motion but doesn't involve the kill ring.
